...
#DBS="Heterorhabditis_bacteriophora-1.2.1.contigs.fa cremanei_contigs.100k.L2k.split.fasta combined_worms_supercontigs200808.fasta b_malayi.WS194.100k.L2k.split.fasta meloidogyne_entrez.fasta Mhapla_wgs080929.wgs.100k.L2k.split.fasta pristionchus_pacificus_contigs.fa schmidtea_mediterranea_31_contigs.100k.L2k.split.fasta sma_v3.1.fasta t_spiralis1.0-contigs.fa cjaponica_4.0.1_supercontigs.fa Cbrenneri_PB2801_6.0.1.100k.L2k.split.fasta c_briggsae.100kOL2k.fasta E_multilocularis_contigs_230108.fa S_ratti_contigs.141008 Ascaris_suum.WGS.contigs nembase3.clustalnames nematode_net_4_species_est_clusters.fasta N_brasiliensis_454_contigs_033109.fasta TCIR.supercontigs.Phusion.120209.fasta tfru.100kL2k.fasta btu_mdass_full.100k.L2k.split.fasta"
DBS="brugia_aa_112707.spc.pep c_brenneri.protein.WS200.spc.pep c_briggsae.protein.WS200.spc.pep c_elegans.protein.WS200.spc.pep c_japonica.protein.WS200.spc.pep c_remanei.protein.WS200.spc.pep ppapep.WS200.spc.pep"

if [ "$DOBLAST" -eq 1 ]
then

    for db in $DBS; do 
	BLASTRESULT=$SEARCHES/${SEEDQUERYFILE}_vs_${db}.blp.m8;

	blastall -p blastp -i $SEEDQUERYFILE -d $DBDIR/$db -m8 -o $BLASTRESULT;

	EFILTNAMES=$SEARCHES/${SEEDQUERYFILE}_vs_${db}.blp.e001genes.names;
	awk '$11 < 0.01 { print; }' < $BLASTRESULT  |cut -f2 |sort |uniq > $EFILTNAMES;

	BLASTRESFASTA=$SEQTEMP/${SEEDQUERYFILE}_vs_${db}.blp.e001genes.fasta;
	perl -ne 's/\|$//; s/\|/\\\|/g; s/\./\\\./; print;' < $EFILTNAMES > ${EFILTNAMES}.grepsafe
	$BINDIR/fasta_header_grep.pl -w -f ${EFILTNAMES}.grepsafe $DBDIR/${db} > $BLASTRESFASTA;
    done
fi

if [ "$DOTREES" -eq 1 ]
then
    TOTALFASTA=$SEQTEMP/${SEEDQUERYFILE}.blp.e001genes.fasta
    
    cp $SEEDQUERYFILE $TOTALFASTA
    
    for db in $DBS ; do 

	BLASTRESFASTA=$SEQTEMP/${SEEDQUERYFILE}_vs_${db}.blp.e001genes.fasta;
	cat $BLASTRESFASTA >> $TOTALFASTA;

	#clustalnames
    done

    clustalw $TOTALFASTA
    clustalw -INFILE=$SEQTEMP/`basename $TOTALFASTA .fasta`.aln -BOOTSTRAP=1000   
fi
...
# for -pep -sum
	...
	cat $SEQTEMP/db_init $SEARCHES/${MOTIF}_in_${SEEDQUERYFILE}_vs_${db}.tbn.e01contigs.wise.hitsfasta | perl -e '$l = <STDIN>; chomp $l; $dbinitial=$l; while($l=<STDIN>) { chomp $l; if($l=~/^>([Cc]{1}ontig\S+).*/) {$out=$dbinitial."_Contig"; $l=~s/[Cc]{1}ontig/$out/; } elsif($l=~/^>([Ss]{1}uper[Cc]{1}ontig\S+).*/) {$out=$dbinitial."_supercontig"; $l=~s/[Ss]{1}uper[Cc]{1}ontig/$out/; } elsif($l=~/^>Smp_contig.*/) {$out=$dbinitial."_contig"; $l=~s/Smp_contig/$out/; } elsif($l=~/^>chr/) { $out=$dbinitial."_chr"; $l=~s/>chr/>$out/; } elsif($l=~m/gi\|\d+\|(?:emb|gb)\|/) { $out=$dbinitial."_"; $l=~s/gi\|\d+\|(?:emb|gb)\|/$out/; $l=~s/\|_/_/g;}; print $l,"\n";}' > $SEARCHES/${MOTIF}_in_${SEEDQUERYFILE}_vs_${db}.tbn.e01contigs.wise.tree.fasta
    done
fi
